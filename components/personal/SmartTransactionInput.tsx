...
import { PersonalQRCard } from "@/components/personal/PersonalQRCard"; // Import PersonalQRCard
...
export default function SmartTransactionInput({ compact = false, user: propUser }: SmartTransactionInputProps) {
    ...
    // ... (keep existing handlers: handleQRScan, validateQR, handleSaveParsed, handleSaveManual) ...
    ...
    // ... [Use existing logic for non-compact classes if needed, or define inside render] ...

    if (compact) {
        const pillButtonClasses = "flex items-center justify-center gap-2 px-4 py-2.5 rounded-full bg-white/80 dark:bg-white/10 border border-slate-100 dark:border-white/5 hover:border-emerald-500/50 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-all group";
        const pillIconClasses = "w-3.5 h-3.5 text-emerald-600 group-hover:scale-110 transition-transform duration-300";
        const pillTextClasses = "text-xs font-bold text-[#1B4332] dark:text-emerald-50 whitespace-nowrap";
        ...
        return (
            <GlassCard
                ...
                blur="md"
            >
                <div className="flex flex-col sm:flex-row sm:items-stretch gap-4 sm:gap-6 flex-1">
                    {/* Title Section */}
                    <div className="flex items-center gap-2 px-2 sm:px-4 py-2 sm:border-r border-slate-200/50 dark:border-white/10 sm:pr-6 justify-center sm:justify-start">
                        <div className="w-6 h-6 shrink-0 rounded-full bg-emerald-500 flex items-center justify-center">
                            {/* Wallet icon or similar for Personal Finance */}
                            <PenTool className="w-3.5 h-3.5 text-white stroke-[3px]" />
                        </div>
                        <span className="text-[11px] font-bold text-[#1B4332] uppercase tracking-[0.15em] whitespace-nowrap writing-mode-vertical sm:writing-mode-horizontal">
                            Personal Finance
                        </span>
                    </div>

                    {/* Actions Column */}
                    <div className="flex flex-col justify-center gap-4 w-full sm:w-auto flex-1 py-1">
                        {/* Buttons Row */}
                        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                            ...
                            <Dialog open={isSmsOpen} onOpenChange={setIsSmsOpen}>
                                <DialogTrigger asChild>
                                    <button className={pillButtonClasses}>
                                        <MessageSquareText className={pillIconClasses} />
                                        <span className={pillTextClasses}>SMS</span>
                                    </button>
                                </DialogTrigger>
                            ...
                            >
                                <button className={pillButtonClasses}>
                                    <ScanLine className={pillIconClasses} />
                                    <span className={pillTextClasses}>Scan QR</span>
                                </button>
                            </QRScannerDialog>
                            ...
                            <Dialog open={isManualOpen} onOpenChange={setIsManualOpen}>
                                <DialogTrigger asChild>
                                    <button className={pillButtonClasses}>
                                        <PenTool className={pillIconClasses} />
                                        <span className={pillTextClasses}>Manual</span>
                                    </button>
                                </DialogTrigger>
                            ...
                            {/* Link to Personal Finance Dashboard */}
                            <Link href="/personal" className={pillButtonClasses}>
                                <LayoutDashboard className={pillIconClasses} />
                                <span className={pillTextClasses}>Dashboard</span>
                            </Link>
                        </div>

                        {/* uBank ID Row (Moved Below Buttons) */}
                        {/* uBank ID Row (PhonePe Style) */}
                        <div className="w-full px-4">
                            <PersonalQRCard user={user}>
                                <button className="w-full flex items-center justify-between px-4 py-2 bg-slate-100 dark:bg-white/5 rounded-full border border-slate-200 dark:border-white/10 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors group">
    ...
    const containerClasses = cn(
        "relative overflow-hidden bg-white/60 dark:bg-white/5 border border-slate-200/60 dark:border-white/5 shadow-2xl shadow-emerald-900/5 transition-all duration-300",
        "p-5 rounded-[2rem] sm:rounded-[2.5rem] h-full"
    );

    const buttonClasses = cn(
        "flex flex-col items-center justify-center rounded-2xl bg-slate-100 dark:bg-white/5 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 border border-slate-200 dark:border-white/10 hover:border-emerald-200 dark:hover:border-emerald-500/20 transition-all group aspect-square",
        "gap-2.5 p-3"
    );

    const iconContainerClasses = cn(
        "rounded-full bg-slate-200 dark:bg-white/10 group-hover:bg-emerald-500 group-hover:text-white flex items-center justify-center transition-colors text-slate-600 dark:text-slate-400",
        "w-9 h-9"
    );

    const iconSize = "w-5 h-5";
    const labelClasses = cn(
        "font-bold text-slate-600 dark:text-slate-300 group-hover:text-emerald-700 dark:group-hover:text-emerald-400",
        "text-xs"
    );

    // Default Render (Non-Compact / Personal Page)
    return (
        <div className={containerClasses}>
            {/* Header */}
            <div className="flex flex-col gap-4 relative z-10 mb-auto">
                <div>
            ...
            <div className={cn("flex flex-col relative z-10", "h-full justify-center")}>
                {/* Buttons Grid */}
                    ...
                    <Dialog open={isSmsOpen} onOpenChange={setIsSmsOpen}>
                        <DialogTrigger asChild>
                            <button className={buttonClasses}>
                                ...
                                <span className={labelClasses}>SMS</span>
                            </button>
                        </DialogTrigger>
                        <DialogContent className="sm:max-w-md">
                            {/* ... [Replicated ID content - ideally refactor this out to sub-component to avoid duplication, but for now inline] ... */}
                            {/* Note: I will need to duplicate dialog contents here or refactor. 
                                For safety/speed, duplicating the dialog content structure is safer than massive refactor now.
                                Or better: make the dialogs triggered by state only, and have shared dialogs at bottom.
                            */}
                            <DialogHeader>
                                <DialogTitle>Parse Transaction SMS</DialogTitle>
                                <DialogDescription>Paste your bank SMS to automatically extract details.</DialogDescription>
                    ...
                    <Dialog open={isManualOpen} onOpenChange={setIsManualOpen}>
                        <DialogTrigger asChild>
                            <button className={buttonClasses}>
                                ...
                                <span className={labelClasses}>Manual</span>
                            </button>
                        </DialogTrigger>
                        <DialogContent className="sm:max-w-lg overflow-visible">
                            <DialogHeader>
                ...
                {/* uBank ID Row (PhonePe Style) - Matching Compact View */}
                <div className="w-full mt-4 pt-4 border-t border-slate-200/60 dark:border-white/5">
                    <PersonalQRCard user={user}>
                        <button className="w-full flex items-center justify-between px-4 py-3 bg-slate-100 dark:bg-white/5 rounded-full border border-slate-200 dark:border-white/10 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors group">
