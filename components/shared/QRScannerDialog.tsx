...
interface QRScannerDialogProps {
    ...
    title?: string
    description?: string
    validationFn?: (data: string) => string | null // Returns error message if invalid, null if valid
...
export function QRScannerDialog({
    ...
    onScan,
    title = "Scan QR Code",
    description = "Align QR Code within the frame",
    validationFn,
    shouldCloseOnScan = true
    ...
    const scannerId = 'generic-qr-reader'
                    ...
                    if (devices && devices.length > 0) {
                        await scanner.start(
                            ...
                            async (decodedText) => {
                                try {
                                    if (validationFn) {
                                        const validationError = validationFn(decodedText)
                                        if (validationError) {
    ...
    }, [isOpen, isTimeout, validationFn, onScan, shouldCloseOnScan])
